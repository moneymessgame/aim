...
import { BlueprintContext, ActiveConnection } from './BlueprintContext';
...
export function Node({ 
  ...
  // Получаем глобальный контекст для взаимодействия между блупринтами
  ...
  // Проверяем, есть ли глобальное активное соединение
  const globalActiveConnection = blueprintContext?.activeConnection;
  
  // Определяем, нужно ли подсвечивать точки соединения
  const shouldHighlightPoints = highlightPoints || (globalActiveConnection !== null && !activeConnection);
            ...
            >
              <div 
                className={cn(
                  ...
                  shouldHighlightPoints && "animate-pulse ring-2 ring-green-500 ring-opacity-75"
                )}
                style={{
