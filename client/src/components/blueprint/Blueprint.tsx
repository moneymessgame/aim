...
export function Blueprint({ initialData, onDataChange }: BlueprintProps) {
	...
	const selectedNodes =
		selectedNodesIds.length > 0
			? nodes.filter((node) => selectedNodesIds.includes(node.id))
			: [];
	...
	const [connectionStartPosition, setConnectionStartPosition] = useState<{
		x: number;
		y: number;
	} | null>(null);
	...
	const handleConnectStart = (
		...
		if (activeConnection || globalActiveConnection) {
			...
			const isFromExternalBlueprint =
				!activeConnection && globalActiveConnection;
	...
	useEffect(() => {
		if (
			blueprintContext &&
			blueprintContext.activeConnection &&
			!activeConnection
		) {
	...
	useEffect(() => {
		if (mounted && onDataChange) {
			...
			if (
				nodesJSON !== prevNodesRef.current ||
				connectionsJSON !== prevConnectionsRef.current
			) {
	...
	const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
		...
		if (containerRef.current) {
			...
			if (templateData) {
				try {
					const nodeTemplate = JSON.parse(templateData) as Omit<
						NodeData,
						'id' | 'position'
					>;
					const newNode: NodeData = {
		...
		<div className="flex h-full">
			...
			<div className="flex-1">
				<DndContext sensors={sensors} onDragEnd={handleDragEnd}>
					<div
						ref={containerRef}
						className={`relative w-full h-full min-h-[600px] bg-gray-50 border border-gray-200 rounded-lg overflow-hidden ${
							isOverBlueprint ? 'cursor-grab' : ''
						}`}
