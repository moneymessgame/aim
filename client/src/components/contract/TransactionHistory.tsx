...
import { useSonicTokenContract, formatTokenAmount } from '../../../app/utils/contract'
...
export default function TransactionHistory() {
  ...
  const fetchTransactionHistory = useCallback(async () => {
    ...
    try {
      ...
      console.log(`Запрашиваем события с блока ${fromBlock} до ${currentBlock}`)
      ...
      const txPromises = logs.map(async (log) => {
        ...
        const isTransfer = from !== '0x0000000000000000000000000000000000000000'
      ...
      setError('Не удалось загрузить историю транзакций. Попробуйте перезагрузить страницу.')
    } finally {
      setLoading(false)
