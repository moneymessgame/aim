...
import { SONIC_TOKEN_ABI, useSonicTokenContract, formatTokenAmount } from '../../../app/utils/contract'
...
export default function ContractInfo() {
  ...
  const fetchContractInfo = useCallback(async () => {
    ...
    try {
      ...
      setError('Не удалось загрузить информацию о контракте. Попробуйте перезагрузить страницу.')
    } finally {
      setLoading(false)
    }
  ...
  if (loading) {
    return <div className="flex justify-center items-center py-10 font-montserrat">Загрузка информации о контракте...</div>
  }
    ...
    <div className="font-montserrat">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-medium text-gray-500">Адрес контракта</h3>
            <p className="mt-1 break-all">{SONIC_TOKEN_ADDRESS}</p>
          </div>
          
          <div>
            <h3 className="text-sm font-medium text-gray-500">Название токена</h3>
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-500">Десятичные знаки</h3>
        ...
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-medium text-gray-500">Общее предложение</h3>
            <p className="mt-1">{contractInfo.totalSupply} {contractInfo.symbol}</p>
          </div>
          
          <div>
            <h3 className="text-sm font-medium text-gray-500">Владелец контракта</h3>
          ...
          <div>
            ...
            <p className="mt-1">{contractInfo.userBalance} {contractInfo.symbol}</p>
          </div>
        </div>
      </div>
    </div>
